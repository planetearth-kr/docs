...
| 코드 | HTTP | 설명 |
|------|------|------|
| `NO_DATA_FOUND` | 200 | 조건에 맞는 데이터 없음 |
| `NO_PARAMETER_PROVIDED` | 200 | 파라메터 누락 |
| `MULTIPLE_PARAMETERS_PROVIDED` | 200 | 파라메터 2개 이상 입력 |
| `INVALID_PARAMETERS_PROVIDED` | 200 | 파라메터 형식 오류 |
| `INVALID_NAME_PROVIDED` | 200 | 존재하지 않는 마인크래프트 닉네임 |
| `RATE_LIMITED` | 429 | 요청 횟수 초과 |
| `UPSTREAM_ERROR` | 503 | Mojang API 연결 실패 |
...
| `trustedResidents` | string | 신뢰 주민 목록 |
...
| 파라메터 | 타입 | 설명 |
|----------|------|------|
| `discord` | string | 디스코드 ID (17~19자리 숫자) |
| `uuid` | string | 마인크래프트 UUID |
| `name` | string | 마인크래프트 닉네임 |
...
닉네임, UUID, 또는 경매 ID로 현재 등록된 경매 아이템을 조회합니다.
...
| `itemInfo.displayName` | string \| null | 이름 (없으면 `null`) |
...
현재 경매 중인 모든 아이템을 반환합니다. 만료 일시 기준 오름차순 정렬.
...
닉네임 또는 UUID로 해당 플레이어의 경매 거래 내역을 조회합니다.  
구매자 또는 판매자로 참여한 거래를 최신 순으로 최대 100건 반환합니다.
...
**Example Request**
```
GET https://api.planetearth.kr/auction/transactions?name=Steve
```
